...
static int ti_project(const t_cam_frame *fr, t_vec3 point, int w, int h, int *ox, int *oy)
{
	t_vec3 v;
	float z;
	float xc;
	...
	if (focal <= 0.0f || half_w <= 0.0f || half_h <= 0.0f)
		return (0);
	x_img = (xc * focal) / z;
	y_img = (yc * focal) / z;
...
static void ti_draw_axes(t_toggle_info *ti, const t_cam_frame *fr)
{
	...
	if (!ti_project(fr, v3(0.0f, 0.0f, 0.0f), ti->w, ti->h, &origin_x,
		&origin_y))
		return;
...
void ti_init(t_toggle_info *ti, mlx_t *mlx, mlx_image_t *img)
{
	...
	instance_id = mlx_image_to_window(mlx, ti->overlay, 0, 0);
	if (instance_id < 0)
	{
		mlx_delete_image(mlx, ti->overlay);
		ti->overlay = NULL; free(ti->buffer);
		ti->buffer = NULL;
		return ;
	}
